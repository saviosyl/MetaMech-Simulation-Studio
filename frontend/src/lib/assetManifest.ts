...
function loadInlineManifest(): AssetDef[] {
  ...
  ].filter((asset) => !removedRuntimeIds.has(asset.id));
}

export function getAssetManifest(): AssetDef[] {
  if (!_manifest) {
    const base = loadInlineManifest().filter((asset) => !REMOVED_LIBRARY_ASSET_IDS.has(asset.id));
    if (_runtimeExternalAssets.length > 0) {
      ...
      _manifest = Array.from(dedup.values()).filter((asset) => !REMOVED_LIBRARY_ASSET_IDS.has(asset.id));
    } else {
      _manifest = base;
    }
  }
